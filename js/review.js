...
dataPekerja.forEach(pekerja => {
  ...
  row.innerHTML = `
    ...
    <td>
        ${pekerja.status.toLowerCase() === "waiting" ? `
          <button class="btn btn-success btn-sm me-1" onclick="handleApprove('${pekerja.no}')">Approve</button>
          <button class="btn btn-danger btn-sm" onclick="handleReject('${pekerja.no}')">Reject</button>
        ` : 'Selesai'}
    </td>
